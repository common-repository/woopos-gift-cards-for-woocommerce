 ...
 * @since       1.0.0
 */
class WOOPOSGC_Giftcard_Activation {

    public $plugin_name, $plugin_path, $plugin_file, $has_woo, $wooposgc_base;
    ...
    public function __construct( $plugin_path, $plugin_file ) {
        ...
        // Set plugin name
        $this->plugin_name = 'WooPOS Gift Cards for WooCommerce';
        ...
        foreach( $plugins as $plugin_path => $plugin ) {
            ...
            if( $plugin['Name'] == 'WooCommerce' ) {
                $this->has_woo = true;
                $this->wooposgc_base = $plugin_path;
                break;
            }
        }
    }
    ...
    public function no_woo_nag() {
        ...
        if( $this->has_woo ) {
            $url  = esc_url( wp_nonce_url( admin_url( 'plugins.php?action=activate&plugin=' . $this->wooposgc_base ), 'activate-plugin_' . $this->wooposgc_base ) );
